  ...
  return (
    <div className="bg-slate-50 dark:bg-slate-800/50 dark:bg-slate-900 min-h-screen transition-colors duration-300 py-10">
      <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
        ...
        >
          <motion.h1 variants={fadeIn} className="font-display text-4xl md:text-5xl font-bold text-slate-900 dark:text-slate-50 mb-6">Country Guides</motion.h1>
          <motion.p variants={fadeIn} className="text-lg md:text-xl text-slate-600 dark:text-slate-400 mb-8">Comprehensive guides on visas, living costs, and cultural adaptation.</motion.p>
        ...
        >
          {allCountries.map((country, idx) => (
            <motion.div 
              variants={fadeIn}
              key={country.id} 
              onClick={() => navigate(`/study/${country.id}`)}
