...
import { CheckCircle2, AlertTriangle, ExternalLink, Calendar, Banknote, Briefcase, Globe, Settings, MapPin, ChevronRight, UserCheck } from 'lucide-react';
import { motion } from 'motion/react';
import { Link } from 'react-router-dom';

export default function GermanyDentistContent() {
    ...
    <motion.div variants={containerVariants} initial="hidden" animate="visible" className="mt-12 space-y-12">
      ...
      <motion.div variants={itemVariants} className="bg-amber-50 dark:bg-amber-900/20 border border-amber-200 dark:border-amber-800 rounded-xl p-6 flex items-start shadow-sm">
        ...
        <div>
          ...
          <p className="text-amber-800 dark:text-amber-200">
            Requirements vary by German state. Always verify with the relevant State Dental Chamber (Landeszahnärztekammer) before starting the process.
          </p>
        </div>
      </motion.div>
      ...
      <motion.section variants={itemVariants} className="bg-white dark:bg-slate-800 rounded-[2rem] p-8 md:p-10 shadow-sm border border-slate-100 dark:border-slate-700">
        <h2 className="font-display text-3xl font-bold text-slate-900 dark:text-slate-50 mb-6 flex items-center">
          <Globe className="w-8 h-8 mr-4 text-teal-600 dark:text-teal-400" /> High Demand for Dentists
        </h2>
        <div className="space-y-4 text-slate-600 dark:text-slate-300">
          <p>
            <strong>The Demand:</strong> There is a significant demand for dentists, particularly in rural and semi-urban Germany, as many older practitioners are retiring.
          </p>
          <p>
            <strong>Career Potential:</strong> After securing your license and working for a few years, it is highly feasible to open your own private dental clinic in Germany.
          </p>
          <p>
            <strong>Salary Expectations:</strong> Salaries range from €70,000 to €150,000 depending on specialization, experience, and practice ownership.
          </p>
        </div>
      </motion.section>
      ...
      <motion.section variants={itemVariants} className="bg-white dark:bg-slate-800 rounded-[2rem] p-8 md:p-10 shadow-sm border border-slate-100 dark:border-slate-700">
        <h2 className="font-display text-3xl font-bold text-slate-900 dark:text-slate-50 mb-6 flex items-center">
          <Settings className="w-8 h-8 mr-4 text-teal-600 dark:text-teal-400" /> Approbation Process for Dentists
        </h2>
        <p className="text-slate-600 dark:text-slate-300 mb-6">
          The process is very similar to medical doctors but is managed by the dental chamber (Zahnärztekammer), not the medical chamber.
        </p>
        <div className="space-y-6 relative before:absolute before:inset-0 before:ml-5 before:-translate-x-px md:before:mx-auto md:before:translate-x-0 before:h-full before:w-0.5 before:bg-gradient-to-b before:from-transparent before:via-teal-200 dark:before:via-teal-800 before:to-transparent">
          {[
            { step: '1', title: 'PMDC Certificate', desc: 'Secure your PMDC dental registration certificate from Pakistan.' },
            { step: '2', title: 'HEC Attestation', desc: 'Get your BDS degree fully attested by HEC.' },
            { step: '3', title: 'Translations', desc: 'Certified German translations of all documents.' },
            { step: '4', title: 'Apply to Landeszahnärztekammer', desc: 'Submit application to the State Dental Chamber.' },
            { step: '5', title: 'Equivalence Assessment', desc: 'Degree comparison by the authorities.' },
            { step: '6', title: 'Kenntnisprüfung', desc: 'Required if significant differences are found in curriculum.' },
            { step: '7', title: 'Fachsprachenprüfung', desc: 'Dental German (C1 level) exam.' },
            { step: '8', title: 'Start Working', desc: 'Receive Approbation and begin practicing.' }
          ].map((item, i) => (
        ...
        <div className="mt-8 bg-teal-50 dark:bg-teal-900/20 p-4 rounded-xl border border-teal-100 dark:border-teal-800/30">
          ...
          <p className="text-sm text-teal-700 dark:text-teal-300 mt-1">Usually takes between 6 and 18 months to complete the full pathway.</p>
        </div>
      </motion.section>
          ...
          <div>
            <ul className="space-y-4">
              <li className="flex flex-col bg-slate-50 dark:bg-slate-900 p-4 rounded-lg border border-slate-100 dark:border-slate-700">
                <span className="font-semibold text-slate-900 dark:text-slate-100">Junior Dentist</span>
                <span className="text-teal-600 dark:text-teal-400 font-bold">€60,000 - €80,000 / year</span>
              </li>
              <li className="flex flex-col bg-slate-50 dark:bg-slate-900 p-4 rounded-lg border border-slate-100 dark:border-slate-700">
                <span className="font-semibold text-slate-900 dark:text-slate-100">Experienced Dentist</span>
                <span className="text-teal-600 dark:text-teal-400 font-bold">€80,000 - €120,000 / year</span>
              </li>
              <li className="flex flex-col bg-slate-50 dark:bg-slate-900 p-4 rounded-lg border border-slate-100 dark:border-slate-700">
                <span className="font-semibold text-slate-900 dark:text-slate-100">Own Practice</span>
                <span className="text-teal-600 dark:text-teal-400 font-bold">€100,000 - €200,000 / year</span>
              </li>
            </ul>
             <p className="text-xs text-slate-500 mt-2">*Note: These are gross amounts.</p>
          </div>
          <div>
             ...
             <ul className="space-y-3">
              {[
                ...
                'Pension system (Zahnärzteversorgung)',
      ...
      <motion.div variants={itemVariants} className="grid grid-cols-1 md:grid-cols-2 gap-8">
        <div className="bg-white dark:bg-slate-800 rounded-[2rem] p-8 shadow-sm border border-slate-100 dark:border-slate-700">
          <h2 className="font-display text-2xl font-bold text-slate-900 dark:text-slate-50 mb-6 flex items-center">
            <CheckCircle2 className="w-7 h-7 mr-3 text-teal-600 dark:text-teal-400" /> Specializations in Demand
          </h2>
          <div className="flex flex-wrap gap-2">
            {['Oral Surgery', 'Orthodontics', 'Periodontics', 'Pediatric Dentistry', 'Implantology'].map(spec => (
        ...
        <div className="bg-teal-600 dark:bg-teal-900 rounded-[2rem] p-8 text-white shadow-sm">
          ...
          <ul className="list-disc pl-5 space-y-3 text-teal-50">
            <li><strong>Dentalstellen.de:</strong> Dedicated platform.</li>
            <li><strong>Medi-Jobs.de:</strong> Healthcare job portal.</li>
            <li><strong>Private Clinics:</strong> Direct hiring by private practices looking for associates.</li>
            <li><strong>NHS Equivalents:</strong> Working for state-insured healthcare networks.</li>
          </ul>
        </div>
      </motion.div>
      ...
      <motion.section variants={itemVariants}>
        ...
        <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
          {[
            { title: "Landeszahnärztekammer Directory", url: "https://www.bzaek.de/ueber-uns/landeszahnaerztekammern.html" },
