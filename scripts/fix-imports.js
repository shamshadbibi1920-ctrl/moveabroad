...
for (const relPath of files) {
  ...
  const parts = relPath.split('/').filter(Boolean); // ['src', 'pages', '...']
  const len = parts.length;
  
  let seoPath = '';
  if (len === 3) seoPath = '../components/SEO';
  else if (len === 4) seoPath = '../../components/SEO';
  else if (len === 5) seoPath = '../../../components/SEO';
